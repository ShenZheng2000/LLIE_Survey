...
def measure_dirs(dirA, dirB, use_gpu, verbose=False):
    ...
    paths_A = glob.glob(os.path.join(dirA, '*'))
    paths_B = glob.glob(os.path.join(dirB, '*'))
    ...
    print("======={}=======>".format(dirA))
    print("Average PSNR:", "%.3f" % (psnr))
    print("Average SSIM:", "%.3f" % (ssim))
    print("Average LPIPS:", "%.3f" % (lpips))
